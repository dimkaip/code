...
/// Υπολογισμός θερμοκρασιών πλάκας version 3=τελική
...
float abs_matrix_du(float before[ROWS][COLS],float after[ROWS][COLS])
{
    float du;
    for(int i=0; i<ROWS; i++)
        for(int j=0; j<COLS; j++)du+=abs(after[i][j]-before[i][j]);
    return du;
}
...
//Αλλαγή της m1 σύμφωνα με τον τύπο
    ...
    float du=abs_matrix_du(m0,m_x_secs);
